...
export default function NarrativesSidebar({
  ...
  // Mode: 'slice' (current scrubber position) vs 'destiny' (t = 1.0)
  const [viewMode, setViewMode] = useState('slice');

  // Compute active branches for the selected view mode
  const branchData = useMemo(() => {
    ...
    let detailed;
    if (viewMode === 'slice' && sliceInfo) {
      detailed = sliceInfo;
    } else {
      detailed = simulator.getDetailedState(numActive);
    }
  ...
  }, [simulator, qubits, viewMode, sliceInfo]);

  const { branches, hasPhaseInterference, totalBranches } = branchData;

  return (
    <aside className="w-96 bg-white/95 backdrop-blur-md border-l border-slate-200 shadow-2xl flex flex-col z-20 h-full overflow-hidden text-slate-800 transition-all">
      {/* ── Sidebar Header ─────────────────────────────────────────── */}
      ...
      {/* ── View Mode Controls (Scrubber vs Destiny) ────────────────── */}
      <div className="px-4 py-2.5 bg-slate-100/60 border-b border-slate-200 flex items-center justify-between gap-2 text-xs">
        <span className="text-[11px] font-medium text-slate-500">Evaluation Point:</span>
        <div className="flex items-center bg-slate-200/80 p-0.5 rounded-lg text-[11px] font-medium">
          <button
            ...
            title={`Evaluate states at timeline scrubber (t=${(scrubberPosition ?? 1).toFixed(2)})`}
          >
            Scrubber (t={(scrubberPosition ?? 1).toFixed(2)})
          </button>
          <button
            onClick={() => setViewMode('destiny')}
            className={`px-2.5 py-1 rounded-md transition-all cursor-pointer ${
              viewMode === 'destiny'
                ? 'bg-white text-sky-800 font-semibold shadow-xs'
                : 'text-slate-600 hover:text-slate-900'
            }`}
            title="Evaluate final destiny states (t=1.0)"
          >
            Destiny (t=1.0)
          </button>
        </div>
